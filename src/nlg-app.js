...
</iron-iconset-svg>`;document.head.appendChild(template$1.content);const template$2=html`<iron-iconset-svg name="social" size="24">
<svg><defs>
...
</dom-module>`;document.head.appendChild($_documentContainer$1.content);const template$10=html`<custom-style>
  <style is="custom-style">
    html {
...
`;template$11.setAttribute("style","display: none;");document.head.appendChild(template$11.content);Polymer({_template:html`
    <style>
      :host {
        display: block;
        padding: 8px 0;
...
`,is:"paper-listbox",behaviors:[IronMenuBehavior],hostAttributes:{role:"listbox"}});var currentToast=null;Polymer({_template:html`
    ...
    <slot></slot>
`,is:"paper-toast",behaviors:[IronOverlayBehavior],properties:{fitInto:{type:Object,value:window,observer:"_onFitIntoChanged"},horizontalAlign:{type:String,value:"left"},verticalAlign:{type:String,value:"bottom"},duration:{type:Number,value:3e3},text:{type:String,value:""},noCancelOnOutsideClick:{type:Boolean,value:!0},noAutoFocus:{type:Boolean,value:!0}},listeners:{transitionend:"__onTransitionEnd"},get visible(){Base._warn("`visible` is deprecated, use `opened` instead");return this.opened},get _canAutoClose(){return 0<this.duration&&this.duration!==1/0},created:function(){this._autoClose=null;IronA11yAnnouncer.requestAvailability()},show:function(properties){if("string"==typeof properties){properties={text:properties}}for(var property in properties){if(0===property.indexOf("_")){Base._warn("The property \""+property+"\" is private and was not set.")}else if(property in this){this[property]=properties[property]}else{Base._warn("The property \""+property+"\" is not valid.")}}this.open()},hide:function(){this.close()},__onTransitionEnd:function(e){if(e&&e.target===this&&"opacity"===e.propertyName){if(this.opened){this._finishRenderOpened()}else{this._finishRenderClosed()}}},_openedChanged:function(){if(null!==this._autoClose){this.cancelAsync(this._autoClose);this._autoClose=null}if(this.opened){if(currentToast&&currentToast!==this){currentToast.close()}currentToast=this;this.fire("iron-announce",{text:this.text});if(this._canAutoClose){this._autoClose=this.async(this.close,this.duration)}}else if(currentToast===this){currentToast=null}IronOverlayBehaviorImpl._openedChanged.apply(this,arguments)},_renderOpened:function(){this.classList.add("paper-toast-open")},_renderClosed:function(){this.classList.remove("paper-toast-open")},_onFitIntoChanged:function(fitInto){this.positionTarget=fitInto}});const previousValues=new WeakMap,unsafeHTML=value=>directive(part=>{const previousValue=previousValues.get(part);if(previousValue===value&&isPrimitive(value)){return}const tmp=document.createElement("template");tmp.innerHTML=value;part.setValue(document.importNode(tmp.content,!0));previousValues.set(part,value)});_exports.unsafeHTML=unsafeHTML;_exports.$unsafeHtml={unsafeHTML:unsafeHTML};(function(global,factory){if("function"===typeof define&&define.amd){define(function(){return factory(global,global.document)})}else if("undefined"!==typeof module&&module.exports){module.exports=factory(global,global.document)}else{global.Shake=factory(global,global.document)}})("undefined"!==typeof window?window:void 0,function(window,document){function Shake(options){this.hasDeviceMotion="ondevicemotion"in window;this.options={threshold:15,timeout:1e3};if("object"===typeof options){for(var i in options){if(options.hasOwnProperty(i)){this.options[i]=options[i]}}}this.lastTime=new Date;this.lastX=null;this.lastY=null;this.lastZ=null;if("function"===typeof document.CustomEvent){this.event=new document.CustomEvent("shake",{bubbles:!0,cancelable:!0})}else if("function"===typeof document.createEvent){this.event=document.createEvent("Event");this.event.initEvent("shake",!0,!0)}else{return!1}}Shake.prototype.reset=function(){this.lastTime=new Date;this.lastX=null;this.lastY=null;this.lastZ=null};Shake.prototype.start=function(){this.reset();if(this.hasDeviceMotion){window.addEventListener("devicemotion",this,!1)}};Shake.prototype.stop=function(){if(this.hasDeviceMotion){window.removeEventListener("devicemotion",this,!1)}this.reset()};Shake.prototype.devicemotion=function(e){var current=e.accelerationIncludingGravity,currentTime,timeDifference,deltaX=0,deltaY=0,deltaZ=0;if(null===this.lastX&&null===this.lastY&&null===this.lastZ){this.lastX=current.x;this.lastY=current.y;this.lastZ=current.z;return}deltaX=_Mathabs(this.lastX-current.x);deltaY=_Mathabs(this.lastY-current.y);deltaZ=_Mathabs(this.lastZ-current.z);if(deltaX>this.options.threshold&&deltaY>this.options.threshold||deltaX>this.options.threshold&&deltaZ>this.options.threshold||deltaY>this.options.threshold&&deltaZ>this.options.threshold){currentTime=new Date;timeDifference=currentTime.getTime()-this.lastTime.getTime();if(timeDifference>this.options.timeout){window.dispatchEvent(this.event);this.lastTime=new Date}}this.lastX=current.x;this.lastY=current.y;this.lastZ=current.z};Shake.prototype.handleEvent=function(e){if("function"===typeof this[e.type]){return this[e.type](e)}};return Shake});const sendEvent=(action,details)=>{if("undefined"!==typeof gtag){gtag("event",action,details)}};_exports.sendEvent=sendEvent;_exports.$analytics={sendEvent:sendEvent};const COLORS=[{id:"Dark blue",value:"#1F28BF"},{id:"Nuxeo blue",value:"#0066FF"},{id:"Light blue",value:"#00ADED"},{id:"Teal",value:"#73D2CF"},{id:"Purple",value:"#8400FF"},{id:"Red",value:"#FF0044"},{id:"Orange",value:"#FF9E00"},{id:"Black",value:"#000000"},{id:"Medium gray",value:"#7F8284"},{id:"Light gray",value:"#BCBFBF"},{id:"White",value:"#FFFFFF"}];_exports.COLORS=COLORS;_exports.$colors={COLORS:COLORS};class NLGColorPicker extends LitElement{static get properties(){return{color:{type:Object}}}constructor(){super();this.color={id:"",label:"",fill:"",opacity:""}}render(){let selected;if(this.color.fill){const defaultColor=COLORS.find(color=>color.value.toLowerCase()===this.color.fill.toLowerCase());selected=defaultColor?COLORS.indexOf(defaultColor):COLORS.length}return html$1`
      <style>
        div.color-preview {
          width: 20px;
          height: 20px;
    ...
    `}_renderDefaultColorOptions(){return COLORS.map(color=>html$1`
      <paper-item>
        <div class="color-preview" .style="background-color:${color.value}"></div>
        ${color.id}
      </paper-item>
    `)}_isFillInvalid(color){return!color.fill}_isOpacityInvalid(color){return isNaN(parseFloat(color.opacity))}_onColorSelected(e){if(e.target.selected===COLORS.length){return}this.color.fill=COLORS[e.target.selected].value;this._dispatchColorChangedEvent()}_onFillChanged(e){const fill=e.target.value;if(fill!==this.color.fill){this.color.fill=fill;this._dispatchColorChangedEvent()}}_onOpacityChanged(e){const opacity=e.target.value;if(opacity!==this.color.opacity){this.color.opacity=opacity;this._dispatchColorChangedEvent()}}_dispatchColorChangedEvent(){this.dispatchEvent(new CustomEvent("color-changed",{detail:{color:this.color}}))}}window.customElements.define("nlg-color-picker",NLGColorPicker);const up="ArrowUp",right="ArrowRight",down="ArrowDown",left="ArrowLeft";class NLGRandomizer extends LitElement{static get properties(){return{konamiCode:Array,currentCode:Array}}constructor(){super();this.konamiCode=[up,up,down,down,left,right,left,right,"KeyB","KeyA"];this.currentCode=[];document.addEventListener("keydown",this._onKeys.bind(this));const shakeEvent=new Shake({threshold:15});shakeEvent.start();window.addEventListener("shake",()=>{this._triggerRandomSequence(1)},!1)}render(){}_onKeys(e){this.currentCode.push(e.code);if(this.konamiCode.toString()===this.currentCode.toString()){this.currentCode=[];this.dispatchEvent(new CustomEvent("random-sequence-start"));this._triggerRandomSequence(1);return}const idx=this.currentCode.length-1;if(this.currentCode[idx]!==this.konamiCode[idx]){this.currentCode=[]}}_triggerRandomSequence(interval){if(400>interval){this.dispatchEvent(new CustomEvent("random-sequence-step"));setTimeout(()=>this._triggerRandomSequence(1.2*interval),interval)}else{this.dispatchEvent(new CustomEvent("random-sequence-end"))}}}window.customElements.define("nlg-randomizer",NLGRandomizer);class NLGRouter extends LitElement{static get properties(){return{}}constructor(){super()}getQueryParams(){if(!this.shadowRoot.getElementById("app-location")){return null}const params=this.shadowRoot.getElementById("app-location").queryParams;return!params.template?{}:{template:params.template,size:JSON.parse(params.size),colors:JSON.parse(params.colors)}}setQueryParams(template,size,colors){this.shadowRoot.getElementById("app-location").queryParams={template,size:JSON.stringify(size),colors:JSON.stringify(colors)}}render(){return html$1`
      <app-location id="app-location"></app-location>
    `}firstUpdated(){this.dispatchEvent(new CustomEvent("initial-route",{detail:{params:this.getQueryParams()}}))}}window.customElements.define("nlg-router",NLGRouter);class NLGSizePicker extends LitElement{static get properties(){return{size:{type:Object}}}constructor(){super();this._setSize(0,0)}render(){return html$1`
      <style>
        paper-input {
          width: 8em;
          text-align: center;
      ...
      <paper-input id="height" label="Height" value="${this.size.height}" type="Number" allowed-pattern="[0-9]" min="1"
        @value-changed="${e=>this._onHeightChanged(e)}" .invalid="${1>this.size.height}" always-float-label>
      ...
      <paper-input id="width" label="Width" value="${this.size.width}" type="Number" allowed-pattern="[0-9]" min="1"
        @value-changed="${e=>this._onWidthChanged(e)}" .invalid="${1>this.size.width}" always-float-label>
        <span slot="prefix"></span>
        <div slot="suffix">px</div>
      </paper-input>
    `}_dispatchSizeChangedEvent(){this.dispatchEvent(new CustomEvent("size-changed",{detail:{size:this.size}}))}_onHeightChanged(e){let height=parseInt(e.target.value);height=isNaN(height)?null:height;if(this.size.height!==height){this._setSize(this.size.width,height);this._dispatchSizeChangedEvent()}}_onWidthChanged(e){let width=parseInt(e.target.value);width=isNaN(width)?null:width;if(this.size.width!==width){this._setSize(width,this.size.height);this._dispatchSizeChangedEvent()}}_setSize(width,height){this.size={width,height}}}window.customElements.define("nlg-size-picker",NLGSizePicker);const TEMPLATES=[{id:"Avatar-01",viewBox:`0 0 225 225`,size:{width:250,height:250},colors:[{id:"primary",label:"Primary",fill:"#0066FF",opacity:1},{id:"background",label:"Background",fill:"#FFFFFF",opacity:0}],geometry:`
    ...
    `},{id:"Avatar-02",viewBox:`0 0 16 16`,size:{width:250,height:250},colors:[{id:"primary",label:"Primary",fill:"#FFFFFF",opacity:1},{id:"secondary",label:"Secondary",fill:"#1F28BF",opacity:1},{id:"background",label:"Background",fill:"#FFFFFF",opacity:0}],geometry:`
      <defs>
        <mask id="mask">
    ...
    `},{id:"Avatar-03",viewBox:`0 0 16 16`,size:{width:250,height:250},colors:[{id:"primary",label:"Primary",fill:"#0066FF",opacity:1},{id:"secondary",label:"Secondary",fill:"#73D2CF",opacity:1},{id:"background",label:"Background",fill:"#FFFFFF",opacity:0}],geometry:`
      <defs>
        <mask id="mask">
          <rect fill="white" width="100%" height="100%"/>
    ...
    `},{id:"NUXEO-LOGO-1",viewBox:`0 0 362.98 68.06`,size:{width:533,height:100},colors:[{id:"primary",label:"Primary",fill:"#1F28BF",opacity:1},{id:"secondary",label:"Secondary",fill:"#0066FF",opacity:1},{id:"background",label:"Background",fill:"#FFFFFF",opacity:0}],geometry:`
    ...
    `},{id:"NUXEO-LOGO-3",viewBox:`0 0 215.52 214.28`,size:{width:250,height:250},colors:[{id:"primary",label:"Primary",fill:"#1F28BF",opacity:1},{id:"secondary",label:"Secondary",fill:"#0066FF",opacity:1},{id:"background",label:"Background",fill:"#FFFFFF",opacity:0}],geometry:`
    ...
    `},{id:"NUXEO-LOGO-3-BL",viewBox:`0 0 215.52 214.28`,size:{width:250,height:250},colors:[{id:"primary",label:"Primary",fill:"#1F28BF",opacity:1},{id:"secondary",label:"Secondary",fill:"#0066FF",opacity:1},{id:"background",label:"Background",fill:"#FFFFFF",opacity:0}],geometry:`
      <polygon class="primary" points="0 146.22 0 160.04 20.2 180.25 0 200.45 0 214.28 13.82 214.28 34.03 194.07 54.23 214.28 68.06 214.28 68.06 200.45 47.85 180.25 68.06 160.04 68.06 146.22 54.23 146.22 34.03 166.42 13.82 146.22 0 146.22"/>
      <polygon class="secondary" points="51.05 73.26 51.05 125.02 17.02 125.02 17.02 73.26 0 73.26 0 124.31 17.02 141.32 68.06 141.32 68.06 73.26 51.05 73.26"/>
      <polygon class="secondary" points="51.05 0 0 0 0 68.06 17.02 68.06 17.02 16.31 51.05 16.31 51.05 68.06 68.06 68.06 68.06 17.02 51.05 0"/>
      <path class="secondary" d="M198.51,146.22h-51v51l17,17h51v-51Zm0,51.75h-34V162.52h34Z"/>
      <polygon class="secondary" points="141.79 162.52 141.79 146.22 73.73 146.22 73.73 214.28 141.79 214.28 141.79 197.97 90.75 197.97 90.75 188.16 141.08 188.16 141.08 171.97 90.75 171.97 90.75 162.52 141.79 162.52"/>
    `},{id:"NUXEO-LOGO-3-TR",viewBox:`0 0 215.52 214.28`,size:{width:250,height:250},colors:[{id:"primary",label:"Primary",fill:"#1F28BF",opacity:1},{id:"secondary",label:"Secondary",fill:"#0066FF",opacity:1},{id:"background",label:"Background",fill:"#FFFFFF",opacity:0}],geometry:`
      <polygon class="primary" points="147.46 0 147.46 13.82 167.67 34.03 147.46 54.23 147.46 68.06 161.29 68.06 181.49 47.85 201.7 68.06 215.52 68.06 215.52 54.23 195.32 34.03 215.52 13.82 215.52 0 201.7 0 181.49 20.2 161.29 0 147.46 0"/>
      <polygon class="secondary" points="124.78 0 124.78 51.75 90.75 51.75 90.75 0 73.73 0 73.73 51.05 90.75 68.06 141.79 68.06 141.79 0 124.78 0"/>
      <polygon class="secondary" points="51.05 0 0 0 0 68.06 17.02 68.06 17.02 16.31 51.05 16.31 51.05 68.06 68.06 68.06 68.06 17.02 51.05 0"/>
      <path class="secondary" d="M198.51,146.22h-51v51l17,17h51v-51Zm0,51.75h-34V162.52h34Z"/>
      <polygon class="secondary" points="215.13 89.18 215.13 72.88 147.07 72.88 147.07 140.93 215.13 140.93 215.13 124.63 164.09 124.63 164.09 114.82 214.43 114.82 214.43 98.63 164.09 98.63 164.09 89.18 215.13 89.18"/>
    `},{id:"NUXEO-X",viewBox:`0 0 450 450`,size:{width:250,height:250},colors:[{id:"primary",label:"Primary",fill:"#0066FF",opacity:1},{id:"background",label:"Background",fill:"#FFFFFF",opacity:0}],geometry:`
    ...
    `}];_exports.TEMPLATES=TEMPLATES;const getRenderableTemplate=(templateId,size,colors)=>{if(!templateId||!size){return""}const template=TEMPLATES.find(template=>template.id===templateId);let styles="";colors.forEach(color=>{styles+=`
      .${color.id} {
        fill: ${color.fill};
  ...
  `};_exports.getRenderableTemplate=getRenderableTemplate;_exports.$templates={TEMPLATES:TEMPLATES,getRenderableTemplate:getRenderableTemplate};const DOMURL=window.URL||window.webkitURL||window,renderSVG=(svg,mimetype="image/png")=>{if(!svg.trim()){return}svg=svg.replace(/svg-style>/gmi,"style>");const holder=document.createElement("div");holder.style.display="none";holder.innerHTML=svg;const svgElement=holder.firstElementChild;if(svgElement&&"svg"!=svgElement.nodeName.toLowerCase()){throw new Error("Is not an SVG")}document.body.appendChild(svgElement);svgElement.style.display="";svgElement.style.opacity="1";const w=svgElement.clientWidth,h=svgElement.clientHeight;svgElement.style.display="none";const canvas=document.createElement("canvas");canvas.style.display="none";canvas.width=w*window.devicePixelRatio;canvas.height=h*window.devicePixelRatio;canvas.style.width=w+"px";canvas.style.height=h+"px";const blob=new Blob([svg],{type:"image/svg+xml"}),url=DOMURL.createObjectURL(blob),img=new Image,promise=new Promise((resolve,reject)=>{img.onload=()=>{canvas.getContext("2d").drawImage(img,0,0,canvas.width,canvas.height);DOMURL.revokeObjectURL(url);document.body.removeChild(svgElement);resolve(canvas.toDataURL(mimetype))};img.onerror=()=>{reject("Cannot render SVG")}});img.src=url;return promise};_exports.renderSVG=renderSVG;_exports.$svgRenderer={renderSVG:renderSVG};window.devicePixelRatio=1;class NLGApp extends LitElement{static get properties(){return{_size:{type:Object},_colors:{type:Array},_templateId:{type:String},_showAlphaLayer:{type:Boolean},_updateRouting:{type:Boolean}}}constructor(){super();this._size={width:0,height:0};this._colors=[];this._showAlphaLayer=!0;this._updateRouting=!0}firstUpdated(){this.shadowRoot.getElementById("template-listbox").selected=0}render(){const hasValidSize=this._validateSize(this._size),hasValidColors=this._validateColors(this._colors),content=getRenderableTemplate(this._templateId,this._size,this._colors);if(this._updateRouting&&this._templateId&&hasValidSize&&hasValidColors){this.shadowRoot.getElementById("router").setQueryParams(this._templateId,this._size,this._colors)}return html$1`
      <style>
        :host {
          display: flex;
          flex-direction: column;
    ...
    `}_renderColorPickers(colors){return colors.map(color=>html$1`
      <nlg-color-picker .color="${color}" @color-changed="${this._onColorChanged.bind(this)}"></nlg-color-picker>
    `)}_renderTemplateOptions(){return TEMPLATES.map(template=>html$1`
      <paper-item id="${template.id}">${template.id}</paper-item>
    `)}_renderPreview(size,colors,showAlphaLayer,content){if(!this._validateSize(size)||!this._validateColors(colors)){return}return html$1`
      <div>
    ...
    `}_validateSize(size){return size&&size.width&&0<size.width&&size.height&&0<size.height}_validateColor(color){return color.fill&&!isNaN(parseFloat(color.opacity))}_validateColors(colors){const hasInvalidColor=colors.find(color=>!this._validateColor(color));return!hasInvalidColor}_onTemplateSelected(e){if(this._templateId===e.detail.item.id){return}this._templateId=e.detail.item.id;const template=TEMPLATES.find(template=>template.id===this._templateId);this._size=Object.assign({},template.size);this._colors=template.colors.slice()}_onSizeChanged(e){this._size=e.detail.size}_onColorChanged(e){const changedColor=Object.assign({},e.detail.color);this._colors=this._colors.map(color=>{return color.id===changedColor.id?changedColor:color})}_onInitialRoute(e){const params=e.detail.params;if(params&&params.template){const template=TEMPLATES.find(t=>t.id===params.template);if(template){this._templateId=params.template;this.shadowRoot.getElementById("template-listbox").selected=TEMPLATES.indexOf(template);this._size=params.size;this._colors=params.colors}}}_onRandomSequenceStart(){this._updateRouting=!1}_onRandomSequenceStep(){this._setRandomColors()}_onRandomSequenceEnd(){this._setRandomColors();this._updateRouting=!0}_onShowLayerChanged(e){this._showAlphaLayer=e.target.checked}_onDownloadClick(svg$$1){renderSVG(svg$$1).then(url=>{const link=document.createElement("a");link.setAttribute("hidden",!0);link.download=`${this._templateId}-${this._size.height}x${this._size.width}`;link.href=url;sendEvent("download",{templateId:this._templateId,height:this._size.height,width:this._size.width});link.click()})}_onShareClick(){const input=document.createElement("input");document.body.appendChild(input);input.value=window.location.href;input.select();document.execCommand("copy");document.body.removeChild(input);this.shadowRoot.getElementById("toastShare").open()}_setRandomColors(){const defaultColors=COLORS.slice();this._colors=this._colors.map(_color=>{const color=Object.assign({},_color),defaultColor=defaultColors.splice(_Mathfloor(Math.random()*defaultColors.length),1)[0];color.opacity=1;color.fill=defaultColor.value;return color})}}window.customElements.define("nlg-app",NLGApp)});